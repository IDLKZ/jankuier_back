...
class AcademyGroupScheduleCDTO(BaseModel):
    ...
    reschedule_start_at: DTOConstant.StandardNullableDateTimeField(
        description="Новое время начала при переносе"
    )
    reschedule_end_at: DTOConstant.StandardNullableDateTimeField(
        description="Новое время окончания при переносе"
    )
    is_active: DTOConstant.StandardBooleanTrueField(
        description="Флаг активности расписания"
    )
    is_canceled: DTOConstant.StandardBooleanFalseField(
        description="Тренировка отменена"
    )
...
class AcademyGroupScheduleUpdateDTO(BaseModel):
    ...
    training_date: (
        DTOConstant.StandardDateField(description="Дата тренировки") | None
    ) = None
    ...
    reschedule_start_at: (
        DTOConstant.StandardNullableDateTimeField(
            description="Новое время начала при переносе"
        )
        | None
    ) = None
    ...
    is_active: (
        DTOConstant.StandardBooleanTrueField(description="Флаг активности расписания")
        | None
    ) = None
    is_canceled: (
        DTOConstant.StandardBooleanFalseField(description="Тренировка отменена") | None
    ) = None
    is_finished: (
        DTOConstant.StandardBooleanFalseField(description="Тренировка завершена") | None
    ) = None
...
class AcademyGroupScheduleBulkCDTO(BaseModel):
    ...
    schedules: list[dict] = (
        []
    )  # [{"training_date": "2025-01-01", "start_at": "09:00", "end_at": "10:00"}]
