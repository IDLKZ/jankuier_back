...
class SMSC(object):
	...
	def send_sms_mail(self, phones, message, translit=0, time="", id=0, format=0, sender=""):
		server = smtplib.SMTP(app_config.smtp_server)

		if app_config.smsc_debug:
			server.set_debuglevel(1)

		if app_config.smtp_login:
			server.login(app_config.smtp_login, app_config.smtp_password)

		server.sendmail(app_config.smtp_from, "send@send.smsc.kz", "Content-Type: text/plain; charset=" + app_config.smsc_charset + "\n\n" + \
							app_config.smsc_login + ":" + app_config.smsc_password + ":" + str(id) + ":" + time + ":" + str(translit) + "," + \
	...
	def _smsc_send_cmd(self, cmd, arg=""):
		url = ifs(app_config.smsc_https, "https", "http") + "://smsc.kz/sys/" + cmd + ".php"
		_url = url
		arg = ifs(app_config.smsc_login, "login=" + quote(app_config.smsc_login) + "&psw=", "apikey=") + quote(app_config.smsc_password) + "&fmt=1&charset=" + app_config.smsc_charset + "&" + arg
		...
		while ret == "" and i <= 5:
			...
			try:
				if app_config.smsc_post or len(arg) > 2000:
					data = urlopen(url, arg.encode(app_config.smsc_charset))
				else:
					data = urlopen(url + "?" + arg)

				ret = str(data.read().decode(app_config.smsc_charset))
			except:
				ret = ""
