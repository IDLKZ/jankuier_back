...
from app.adapters.dto.request_material.request_material_dto import RequestMaterialUpdateDTO, RequestMaterialWithRelationsRDTO
...
class UpdateRequestMaterialCase(BaseUseCase[RequestMaterialWithRelationsRDTO]):
    ...
    async def validate(self, id: int, dto: RequestMaterialUpdateDTO, file: UploadFile | None = None) -> None:
        if not id or id <= 0:
    ...
    async def transform(self, dto: RequestMaterialUpdateDTO, file: UploadFile | None = None):
