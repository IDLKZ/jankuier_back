...
from app.adapters.dto.field.field_dto import (
...
class UpdateFieldMainPhotoCase(BaseUseCase[FieldWithRelationsRDTO]):
    """
    Класс Use Case для обновления главного изображения поля.
    ...
    Атрибуты:
        ...
        model (FieldEntity | None): Модель поля для обновления.

    Методы:
        execute(id: int, file: UploadFile) -> FieldWithRelationsRDTO:
            Выполняет обновление главного изображения поля.
    ...
    async def execute(self, id: int, file: UploadFile) -> FieldWithRelationsRDTO:
        """
        Выполняет операцию обновления главного изображения поля.

        Args:
            id (int): Идентификатор поля.
            file (UploadFile): Файл изображения поля.

        Returns:
            FieldWithRelationsRDTO: Обновленный объект поля с отношениями.

        Raises:
            AppExceptionResponse: Если поле не найдено или файл не валиден.
        """
        await self.validate(id=id, file=file)
        dto = await self.transform(file=file)

        # Обновляем поле используя repository.update
        self.model = await self.repository.update(obj=self.model, dto=dto)

        # Получаем обновленное поле с отношениями
    ...
    async def validate(self, id: int, file: UploadFile) -> None:
        """
        Валидирует данные перед обновлением главного изображения поля.
    ...
    async def transform(self, file: UploadFile) -> FieldUpdateDTO:
        """
        Трансформирует данные перед обновлением главного изображения поля.

        Args:
            file (UploadFile): Файл изображения для обновления.

        Returns:
            FieldUpdateDTO: DTO с обновленным image_id.
        """
        # Определение папки для загрузки изображений полей
        self.upload_folder = f"fields/images/{self.model.value}"
        ...
        return FieldUpdateDTO(image_id=file_entity.id)