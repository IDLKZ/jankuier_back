...
from app.adapters.dto.user_code_verification.user_code_verification_dto import (
    UserCodeVerificationResultRDTO, UserCodeResetPasswordResultRDTO,
)
from app.adapters.repository.user.user_repository import UserRepository
from app.adapters.repository.user_code_reset_password_repository import UserCodeResetPasswordRepository
...
class VerifySmsResetCodeCase(BaseUseCase[UserCodeResetPasswordResultRDTO]):
    def __init__(self, db: AsyncSession):
        self.db = db
        self.user_repository = UserRepository(db)
        self.user_code_verification_repository = UserCodeResetPasswordRepository(db)

    async def execute(self, phone:str,new_password: str, code: str) -> UserCodeResetPasswordResultRDTO:
        ...
        if verification:
            ...
            if expiration_time > now:
                ...
                if code == expected_code:
                    ...
                    await self.user_repository.update(user, {"password_hash": get_password_hash(new_password)})
                    result = True
