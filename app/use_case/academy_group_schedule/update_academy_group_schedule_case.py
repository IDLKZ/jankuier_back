...
class UpdateAcademyGroupScheduleCase(
    ...
    async def validate(self, id: int, dto: AcademyGroupScheduleUpdateDTO) -> None:
        ...
        reschedule_start = (
            dto.reschedule_start_at
            if dto.reschedule_start_at is not None
            else model.reschedule_start_at
        )
        reschedule_end = (
            dto.reschedule_end_at
            if dto.reschedule_end_at is not None
            else model.reschedule_end_at
        )
        ...
        if any([dto.training_date, dto.start_at, dto.end_at]):
            training_date = (
                dto.training_date
                if dto.training_date is not None
                else model.training_date
            )
